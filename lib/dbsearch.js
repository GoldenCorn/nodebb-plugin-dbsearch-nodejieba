...
function tokenize(text) {
  ...
  if (text.length < 500) return result.join(" ") + " " + text;
...
search.init = async function (params) {
  ...
  params.router.get(
    "/api/admin/plugins/dbsearch",
    params.middleware.applyCSRF,
    renderAdmin
  );

  params.router.post(
    "/api/admin/plugins/dbsearch/save",
    params.middleware.applyCSRF,
    save
  );

  pluginConfig = await getPluginData();
  await searchModule.createIndices(
    convertLanguageName(
      pluginConfig ? pluginConfig.indexLanguage || "en" : "en"
    )
  );
...
search.actionPostsPurge = function (data) {
  searchRemove(
    "post",
    data.posts.map((p) => p && p.pid)
  );
};

search.actionPostMove = async function (data) {
  const topicData = await topics.getTopicFields(data.post.tid, [
    "cid",
    "deleted",
  ]);
...
search.actionPostChangeOwner = async function (hookData) {
  ...
  const posts = hookData.posts.filter(
    (p) => tidToTopic[p.tid] && !tidToTopic[p.tid].deleted
  );
  posts.forEach((p) => {
...
search.filterSearchQuery = async function (data) {
  ...
  const limit =
    data.index === "post" ? pluginConfig.postLimit : pluginConfig.topicLimit;
  const query = {};
...
search.filterSearchTopic = async function (hookData) {
  ...
  hookData.ids = hookData.ids.concat(
    postData
      .filter((p) => p && p.tid === parseInt(hookData.tid, 10))
      .map((p) => p.pid)
  );
  return hookData;
};
...
search.reindex = async function () {
  ...
  await Promise.all([reIndexTopics(), reIndexPosts()]);
  await db.setObject("nodebb-plugin-dbsearch", {
    working: 0,
  });
};
...
async function topicsSave(topics) {
  ...
  const result = await plugins.hooks.fire("filter:search.indexTopics", {
    data: data,
    tids: tids,
    topics: topics,
  });
...
async function postsSave(posts) {
  ...
  const result = await plugins.hooks.fire("filter:search.indexPosts", {
    data: data,
    pids: pids,
    posts: posts,
  });
...
async function reIndexTids(tids) {
  ...
  await Promise.all([topicsSave(topicData), reIndexTopicsPids(topicData)]);
}
...
async function save(req, res) {
  if (
    utils.isNumber(req.body.postLimit) &&
    utils.isNumber(req.body.topicLimit)
  ) {
    const data = {
...
async function getGlobalAndPluginData() {
  ...
  const languageSupported =
    nconf.get("database") === "mongo" || nconf.get("database") === "postgres";
  const languages = Object.keys(languageLookup).map((code) => ({
    name: languageLookup[code],
    value: code,
    selected: false,
  }));
  ...
  plugin.topicsIndexed =
    plugin.topicsIndexed > plugin.topicCount
      ? plugin.topicCount
      : plugin.topicsIndexed;
  plugin.postsIndexed =
    plugin.postsIndexed > plugin.postCount
      ? plugin.postCount
      : plugin.postsIndexed;
  plugin.languageSupported = languageSupported;
  plugin.languages = languages;
  ...
  plugin.allCategories.forEach((category) => {
    category.selected =
      category && plugin.excludeCategories.includes(String(category.value));
  });
...
async function getProgress() {
  ...
  const topicsPercent = topicCount
    ? (pluginData.topicsIndexed / topicCount) * 100
    : 0;
  const postsPercent = postCount
    ? (pluginData.postsIndexed / postCount) * 100
    : 0;
  return {
    topicsPercent: Math.max(0, Math.min(100, topicsPercent.toFixed(2))),
    postsPercent: Math.max(0, Math.min(100, postsPercent.toFixed(2))),
    topicsIndexed:
      topicsPercent >= 100 ? topicCount : Math.max(0, pluginData.topicsIndexed),
    postsIndexed:
      postsPercent >= 100 ? postCount : Math.max(0, pluginData.postsIndexed),
...
socketAdmin.plugins.dbsearch.changeLanguage = async function (
  socket,
  language
) {
...
admin.menu = function (custom_header, callback) {
  custom_header.plugins.push({
    ...
    icon: "fa-search",
    name: "DB Search (Nodejieba version)",
  });
